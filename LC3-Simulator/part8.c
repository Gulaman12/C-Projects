...
int main(int argc, char *argv[]){
	if (argc == 2){
		...
		int i = 0, brPosition;//index of array
		uint16_t andFormat,andResult, addFormat, addResult;
		uint16_t condition,branch;
		...
		for(i;i<(counter);i++){//will go through array to check if any instruction is ld
			...
			if (check == ADD){
				getDr = instruction >>9;
				getDr = getDr & 0x7;
				addFormat = instruction >>5;
				addFormat = addFormat &0x1;
				if (addFormat == 0){
					src1 = instruction >> 6;
					src1 = src1 & 0x7;
					src2 = instruction & 0x7;
					addResult = directRegister[src1] + directRegister[src2];
					}
				else{
					src1 = instruction >> 6;
					src1 = src1 & 0x7;
					imm5 =instruction & 0x1f;
					if((imm5>>4) == 0){
						addResult = directRegister[src1] + imm5;}
					else{
						imm5 = (~(imm5&0xf)+1)+14;
						if(imm5 == 0){
							imm5 = -16;}
				...
				directRegister[getDr] = addResult;
				ccValue = addResult;
				}
			if (check == BR){
				brPosition = i;
				condition = instruction >>9;
